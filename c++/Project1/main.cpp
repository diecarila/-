...
int main()
{
	...
	// 정적변수(static)  -> data 영역
	// data 영역 특징: 실행될때 생성되고 종료될때 사라짐, 스택과 다른 영역
	// 생성된, 선언된 곳에서만 사용이 가능하다
	
	// 전역변수와 정적변수가 꽤 닮았는데 왜 정적변수를 쓰냐?
	// : 전역변수는 모든 함수에서 접근이 가능하기 때문에 실수로 값이 수정될 가능성이 있음.
	// 이를 방지하기 위해 정적변수를 사용하는것.

	static int g_iStatic = 0;  // -> main 함수 안에서 선언됨
	...
	int i = 100;
	int* pInt = &i; // int 방식으로 접근할거니까 int만 넣어라(?)
